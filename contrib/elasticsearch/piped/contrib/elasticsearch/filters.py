...
from piped.contrib.elasticsearch.exceptions import QueryParameterError
from piped.contrib.elasticsearch.utils import ESRange, encode_json
...
class Filter(object):
    ...
    def _unserialize(self, data):
        ...
        name = data.keys()[0]
        filter = self._find_filter_by_name(name)
        return filter.unserialize(data.values()[0])

    @classmethod
    def _find_filter_by_name(cls, name):
...
class FilterList(Filter):
    ...
    def unserialize(cls, data):
        filters = list()
        for filter in data:
            filters.append(Filter._unserialize(filter))
...
class NotFilter(Filter):
    ...
    @classmethod
    def unserialize(cls, data):
        filter = Filter._unserialize(data)
        return cls(filter=filter)

class RangeFilter(Filter):
    _internal_name = 'range'

    def __init__(self, ranges=None, **kwargs):
        super(RangeFilter, self).__init__(**kwargs)

        self.ranges = ranges or dict()
    ...
    def serialize(self):
        ...
        ranges = dict([(name, r.serialize()) for name, r in self.ranges.items()])
        return {"range":ranges}

    @classmethod
    def unserialize(cls, data):
        for name, range in data.items():
            data[name] = ESRange.unserialize(range)

        print 'unserialied', data
...
class TermFilter(Filter):
    ...
    def unserialize(self, data):
        data = data.copy()
        _name = data.pop('_name', None)
        filter = TermFilter(_name=_name)
