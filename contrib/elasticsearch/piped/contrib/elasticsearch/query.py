...
from piped.contrib.elasticsearch.utils import ESJsonEncoder, clean_string, ESRange

from piped.contrib.elasticsearch.facets import Facet
from piped.contrib.elasticsearch.filters import Filter

from piped.contrib.elasticsearch.highlight import HighLighter
from piped.contrib.elasticsearch.scriptfields import ScriptFields

from piped.contrib.elasticsearch.exceptions import InvalidQuery, InvalidParameterQuery, QueryError, ScriptFieldsError
...
class Search(object):
    ...
    def serialize(self):
        ...
        if self.facets:
            ...
            for name, facet in self.facets.items():
                res['facets'][name] = facet.serialize()
        return res
    ...
    def unserialize(cls, data):
        ...
        if 'query' in data:
            data['query'] = Query._unserialize(data['query'])

        if 'filter' in data:
            data['filter'] = Filter._unserialize(data['filter'])

        if 'highlight' in data:
            data['highlight'] = HighLighter.unserialize(data['highlight'])
#
        #if 'script_fields' in data:
        #    data['script_fields'] = ScriptFields.unserialize(data['script_fields'])
#
        if 'facets' in data:
            facets = data['facets']
            for key, value in facets.items():
                facets[key] = Facet._unserialize(value)

        if 'script_fields' in data:
            data['script_fields'] = ScriptFields._unserialize(data['script_fields'])
...
class Query(object):
    ...
    def replace_with(self, another_query):
        assert self.parents is not None
        for parent in self.parents:
            parent.replace_query(self, another_query)

    def replace_query(self, a, b):
        """ Replace our sub-query a with the new query b. """
        raise NotImplementedError()

    def add_child_query(self, query):
        if query is None:
            return
        query.add_parent(self)

    def remove_child_query(self, query):
        if query is None:
            return
        query.remove_parent(self)

    def add_parent(self, parent):
        self.parents.add(parent)

    def remove_parent(self, parent):
        sef.parents.remove(parent)
    ...
    def _unserialize(self, data):
        ...
        name = data.keys()[0]
        filter = self._find_query_by_name(name)
        return filter.unserialize(data.values()[0])

    @classmethod
    def _find_query_by_name(cls, name):
        for key, value in globals().items():
            if isinstance(value, type) and issubclass(value, Query):
                if getattr(value, '_internal_name', Ellipsis) == name:
                    return value
        raise Exception('No such query type: %s' % name)
...
class HasChildQuery(Query):
    ...
    def __init__(self, type, query, _scope=None, **kwargs):
        ...
        self._scope = _scope
        self.query = query
        self.add_child_query(self.query)
...
class NestedQuery(Query):
    ...
    def replace_query(self, a, b):
        self.query = b
        self.remove_child_query(a)
        self.add_child_query(b)
...
class DisMaxQuery(Query):
    ...
    def add(self, query):
        # TODO: ensure query not added twice
        if isinstance(query, list):
            self.queries.extend(query)
            for q in query:
                self.add_child_query(q)
        else:
...
class FilteredQuery(Query):
    ...
    @classmethod
    def unserialize(cls, data):
        query = Query._unserialize(data['query'])
        filter = Filter._unserialize(data['filter'])
        return cls(query=query, filter=filter)
...
class TermQuery(Query):
    ...
    def unserialize(self, data):
        ...
        assert len(data.keys()) == 1
        field = data.keys()[0]
        value_definition = data.values()[0]
        boost = None
        value = value_definition
        if isinstance(value_definition, dict):
            value = value_definition.get('value', value_definition.get('term', None))
            boost = value_definition.get('boost', None)

        for field, value in data.items():
            query .add(field, value, boost)
        print 'gots', query
        return query
...
class BoostingQuery(Query):
    ...
    def __init__(self, positive=None, negative=None, negative_boost=0.0, **kwargs):
        super(BoostingQuery, self).__init__(**kwargs)

        self.positive = positive
        self.add_child_query(self.positive)

        self.negative = negative
        self.add_child_query(self.negative)
    ...
    def serialize(self):
        data = dict()
        data['positive'] = self.positive.serialize()
        data['negative'] = self.negative.serialize()
        data['negative_boost'] = self.negative_boost

        return {self._internal_name: data}

    @classmethod
    def unserialize(cls, data):
        data['positive'] = Query._unserialize(data['positive'])
        data['negative'] = Query._unserialize(data['negative'])
        return BoostingQuery(**data)
